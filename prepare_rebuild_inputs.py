...
def resolve_working_export(source_root: Path, cohort: str) -> Path:
    """Resolve a private cohort export without embedding a machine path.

    `export_working_dta_to_csv.py` writes to `working_exports/`.  The two
    alternative locations retain compatibility with the locked local rerun and
    with users who provide a directory containing only the six authorised
    cohort exports.
    """
    candidates = [
        source_root / "working_exports" / f"{cohort}.csv",
        source_root / "csv 版本 清洗后" / f"{cohort}.csv",
        source_root / f"{cohort}.csv",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate
    rendered = "\n".join(f"- {candidate}" for candidate in candidates)
    raise FileNotFoundError(f"No authorised working export found for {cohort}. Checked:\n{rendered}")
...
def prepare_cohort(cohort: str, source_root: Path) -> tuple[pd.DataFrame, dict[str, object]]:
    source = resolve_working_export(source_root, cohort)
